...
public class VolumeService extends Service {
    ...
    public void onCreate() {
        ...
        startForeground(1, notificationService(this, getResources(), ""));
    }
    ...
    private void notificationChannel() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            ...
            notificationManagerCompat = MainActivity.getNotificationManagerInstance();
            notificationManagerCompat.createNotificationChannel(notificationChannel);
        }
    }

    public static void updateNotification(Context context, Resources resources, int current) {
        String volumeLabel = "Volume: " + current + "%";
        if (ActivityCompat.checkSelfPermission(context, android.Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
            return;
        }
        notificationManagerCompat.notify(1, notificationService(context, resources, volumeLabel));
    }

    private static Notification notificationService(Context context, Resources resources, String currentVolume) {
